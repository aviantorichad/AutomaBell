...
?>
<script type="text/javascript">function validateTime(){var data=document.getElementById("txt_jam").value;if(data!=null || data!=""){var res=data.split(':');if(res.length==2){var hh=res[0];var mm=res[1];if(hh>=0&&hh<=23){if(mm>=0&&mm<=59){return true;}else{alert("Isian menit belum benar.");return false;}}else{alert("Isian jam  belum benar.");return false;}}else{alert("Isian jam  belum benar.");return false;}}else{alert("Isian jam  belum benar.");return false;}}</script>
...
<form method="post" name="jadwal" action="saving.php" onsubmit="return validateTime()">
	<select name="txt_hari" id="txt_hari">
		<?php
		$qday = mysql_query("SELECT * FROM hari") or die("gagal query [".mysql_error()."] <a href='javascript:history.back()'>back</a>");
		while($rday = mysql_fetch_array($qday)){
			$hari_id = $rday['id'];
			$hari_nm = $rday['hari'];
		?>
	...
	<select name="txt_audio" id="txt_audio" style="max-width:250px">
		<?php
		$qtab = mysql_query("SELECT * FROM audio ORDER by file ASC") or die("gagal query [".mysql_error()."] <a href='javascript:history.back()'>back</a>");
...
<table cellspacing="0">
	...
	<?php
	$no = 1;
	$qtab = mysql_query("SELECT * FROM resume ORDER BY hari ASC, jam ASC") or die("gagal query [".mysql_error()."] <a href='javascript:history.back()'>back</a>");
	while($rtab = mysql_fetch_array($qtab)){
		$res_id = $rtab['id'];
		$res_hr = $rtab['hari'];
